...
import { recipeName } from "../../redux/action"
...
const Search = ({setPage, setIndex})=>{
    ...
    return(
        <div className={styles.search_conteiner}>
            <input className={styles.buscarInput} onChange={handleChange} value={name} type='search' />
            <button className={styles.buscarClick} onClick={()=>{onSearch(name)}}>
